...
export class DeutscheWohnenScraper extends BaseBrowserScraper {
    readonly sourceId = "deutsche-wohnen";
    readonly scheduleCron: string = "0 1 * * *"; // Run once a day at 1 AM
    ...
    private readonly BASE_URL = "https://www.deutsche-wohnen.com";
    ...
    async scrape(context: ScrapeContext): Promise<RawScrapedListing[]> {
        ...
        try {
            ...
            logger.info("Navigating to Deutsche Wohnen listings page");
    ...
    transform(raw: RawScrapedListing): StandardListing {
        ...
        if (typeof d.shareUrl === 'string' && d.shareUrl !== '') {
            ...
            listingUrl = `${this.BASE_URL}/mieten/mietangebote/${d.slug}`;
        }
    ...
    private extractCity(postCodeAndCity: string): string | null {
        ...
        const match = /^\d{5}\s+(\S+)/u.exec(postCodeAndCity);
        ...
        for (const section of sections) {
            ...
            for (const row of rows) {
                ...
                if (r.label === 'Geschoss' && typeof r.value === 'string') {
                    const match = /^(\d+)\./u.exec(r.value.trim());
